...
for d=1:length(ev_labels)
    if ev_labels(d) == 1
        featuresP = [featuresP;features(d,:)]
    end
    
    if ev_labels(d) == -1
        featuresN = [featuresN;features(d,:)]
    end 
end

% train shrinkage LDA classifier (TODO: fill in)
u1 = transpose(mean(featuresP))
u2 = transpose(mean(featuresN))
E1 = cov(featuresP)
E2 = cov(featuresN)

E1 = ((1 - lambda) * E1) + (lambda * eye(size(E1, 1)));
E2 = ((1 - lambda) * E2) + (lambda * eye(size(E2, 1)));

model.w = inv(E1 + E2)*(u2-u1);
model.b = -0.5 * ( transpose(model.w) * (u1 + u2));
